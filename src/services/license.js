...
export async function checkLicense() {
  try {
    ...
    if (expirationDate < now && license.license_type !== LicenseType.PERPETUAL) {
      // 许可证已过期
...
// 检查是否需要显示许可证提醒
export function shouldShowLicenseReminder(licenseStatus) {
  if (!licenseStatus.valid) {
    return true; // 许可证无效，需要提醒
  }

  if (licenseStatus.is_trial && licenseStatus.days_remaining !== null) {
    // 试用期最后3天提醒
    if (licenseStatus.days_remaining <= 3) {
      return true;
    }

    // 试用期使用过半提醒
    if (licenseStatus.days_remaining <= TRIAL_DAYS / 2) {
      return true;
    }
  }
